...
gd = oim.oimGauss(
    fwhm=oim.oimInterp("wl", wl=[3e-6, 3.5e-6, 4e-6], values=[1, 3, 4]),
    f=oim.oimInterp("wl", wl=[3e-6, 4e-6], values=[1, 4]),
)
ud = oim.oimUD(d=0.7, f=1)
ud2 = oim.oimUD(
    x=0, y=5, d=1.5, f=oim.oimInterp("wl", wl=[3e-6, 4e-6], values=[0, 1])
)
ud3 = oim.oimUD(
    x=-5, y=2, d=0.5, f=oim.oimInterp("wl", wl=[3e-6, 4e-6], values=[0.1, 1])
)
eg = oim.oimEGauss(
    fwhm=1,
    elong=1.5,
    pa=oim.oimInterp("wl", wl=[3e-6, 4e-6], values=[20, 60]),
    f=oim.oimInterp("wl", wl=[3e-6, 4e-6], values=[1, 0.1]),
)
er = oim.oimERing(
    din=8,
    dout=oim.oimInterp("wl", wl=[3e-6, 4e-6], values=[15, 20]),
    elong=2,
    pa=0,
)

# NOTE: Link some parameters, actually replacing them
...
# NOTE: Set some spatial frequencies
...
for i, m in enumerate(models):
    # NOTE: Compute visibilities for the models and spatial frequencies
    cf1 = np.abs(m.getComplexCoherentFlux(spf, spf0, wls)).reshape(
        len(wl), len(B)
    )
    cf2 = np.abs(m.getComplexCoherentFlux(spf0, spf, wls)).reshape(
        len(wl), len(B)
    )
    ...
    sc = ax[3, i].scatter(
        spf0 * mas2rad, cf2, c=wls * 1e6, s=0.1, cmap="plasma"
    )
...
for i, m in enumerate(models):
    ...
    for iwl, wli in enumerate(wls2):
        ...
        imshow = ax[iwl, i].imshow(
            ...
            norm=colors.TwoSlopeNorm(vmin=0.0, vcenter=1e-1, vmax=1),
            cmap="plasma",
            interpolation=None,
        )
        ax[iwl, i].text(
...
fig.colorbar(
    imshow, ax=ax[:2, :].ravel().tolist(), label="Normalized Intensity"
)
...
cf2 = np.abs(models[-2].getComplexCoherentFlux(spf0, spf, wls)).reshape(
    len(wl), len(B)
)
