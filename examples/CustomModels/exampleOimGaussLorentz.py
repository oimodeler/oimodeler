...
gl = oim.oimGaussLorentz(
    hlr=10**0.06, flor=0.43, pa=0.12 * u.rad.to(u.deg), elong=1 / 0.83
)
...
fig0, ax0 = sim.plot(
    ["VIS2DATA"], savefig=save_dir / f"ExampleOim{gl.shortname}_prefit.png"
)
...
figWalkers, axeWalkers = fit.walkersPlot(
    savefig=save_dir / f"ExampleOim{gl.shortname}_walkers.png"
)
figCorner, axeCorner = fit.cornerPlot(
    savefig=save_dir / f"ExampleOim{gl.shortname}_corner.png"
)
fig0, ax0 = fit.simulator.plot(
    ["VIS2DATA"], savefig=save_dir / f"ExampleOim{gl.shortname}_fit.png"
)
...
model.showModel(
    512, 0.02, swapAxes=True, fromFT=False, normPow=0.5, colorbar=False
)
plt.savefig(save_dir / f"ExampleOim{gl.shortname}_images.png")

model.showModel(
    512, 0.02, swapAxes=True, fromFT=True, normPow=0.5, colorbar=False
)
