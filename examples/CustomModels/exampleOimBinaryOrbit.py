...
for i in range(nt2-1):
    ax.annotate(f"t={t2[i]:.2f}",(x2[i]+0.1,y2[i]+0.2),color='r',zorder=10,fontsize=8)
ax.axis('equal')
...
fig,ax = plt.subplots(3,nt2,figsize=(nt2*2.5,8),sharex="row",sharey="row")
...
#%% plot colormap of temporal evolution of visibility during 1 period 
...
#Using the starWl interpolator to set a blackbody flux to each component.
...
spf_res = 1.22/(orb.primary.params["d"].value*u.mas.to(u.rad)) # first zero of the primary
...
ax.plot(wl*1e6,fp,label=f"primary: T={orb.primary.params['f'].temp.value}K R={orb.primary.params['f'].radius.value}Ro")
ax.plot(wl*1e6,fs,label=f"secondary: T={orb.secondary.params['f'].temp.value}K R={orb.secondary.params['f'].radius.value}Ro")
ax.legend()
ax.set_yscale("log")
