...
# NOTE: Global simulation parameters
...
# NOTE: Disk geometry in the continuum
fwhmCont = 3  # major-axis FWHM or diameter in D*
fluxDiskCont = 0.5  # disk flux ratio in the continuum

# NOTE: Disk geometry in the line
fwhmLine = 8.0  # circumstellar disk FWHM in the line in D*
EW = 20.0  # line equivalent width in Angstroms


# NOTE: Disk Kinematics
...
# NOTE: Get the internal image-cube, wavelength table and pixSize for this model
...
# NOTE: Plot some images through the line
c = 3e5  # light speed in km/s

dopplerShift = (wl - wl0) / wl0 * c

# NOTE: Indices of the the wavelengths for which the images will be plotted
...
for i in range(nindex):
    ...
    ax[i].text(
        dim / 2,
        dim,
        f"{dopplerShift[index[i]]:.2f} km/s",
        ha="center",
        va="bottom",
        color="w",
    )
...
# NOTE: Same plots but using a model class that allow interpolation between wavelengths and arbitary pixel size
n = 7
...
for i in range(n):
    ...
    ax[i].text(
        dim / 2, 0, f"{wl2[i]*1e9:.2f} nm", ha="center", va="top", color="w"
    )
    ax[i].text(
        dim / 2,
        dim,
        f"{dopplerShift2[i]:.2f} km/s",
        ha="center",
        va="bottom",
        color="w",
    )
...
# NOTE: Set the spatial frequencies for a baseline of 50 m with various orientations
B0 = 50
nB = 200
wls = np.linspace(wl0 - dwl / 2, wl0 + dwl / 2, num=nwl)
...
plt.plot(
    pa,
    maxi * np.abs(np.sin(np.deg2rad(pa))),
    label="abs of sine function",
    ls=":",
)
ax.set_ylabel("Maximum of phase variation (deg)")
