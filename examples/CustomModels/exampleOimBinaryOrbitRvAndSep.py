...
orb = oim.oimBinaryOrbit(
    ...
    T0=2000.6941
    * u.yr,  # Time Periastron passage (MJD by default or decimal year)
    T=10.811
    * u.yr,  # Period (days by default or any compatible astropy unit if specified)
...
def sepRvPrior():
    ...
    chi2_rv = np.sum(
        ((rv - rv_model) / 0.3) ** 2
    )  # we assume 0.1km/s errors on RV
...
fit.walkersPlot(
    chi2limfact=5, savefig=save_dir / "ExampleBinary_delsco_walker.png"
)
fit.cornerPlot(
    discard=2000,
    chi2limfact=5,
    savefig=save_dir / "ExampleBinary_delsco_corner.png",
)
...
ax.scatter(
    x_mod, y_mod, color="grey", label="at epochs of Obs", marker=".", s=20
)
...
for i in range(3):
    ...
    ax.scatter(
        x[idx], y[idx], color=col[i], label=data_text[i], marker=".", zorder=10
    )
...
ax2.plot(
    Time(tmod, format="mjd").byear, rv_mod, color="b", label="Best-fit model"
)
ax2.scatter(
    Time(mjdRv, format="mjd").byear,
    rv,
    color="r",
    label="Data (Miroshnichenko+ 2001)",
    marker=".",
)
ax2.set_ylim(-60, 5)
ax2.legend()
...
figwFit, axFit = fit2.simulator.plotWithResiduals(
    ["VIS2DATA", "T3PHI"], xunit="cycle/arcsec"
)
...
grid.prepare(
    params=[pt.params["x"], pt.params["y"]],
    min=[-10, 70],
    max=[50, 130],
    steps=[0.2, 0.2],
)
...
ax.errorbar(
    ...
    marker=".",
    ls="",
    label="Best-fit position (oimFitterEmcee)",
    zorder=10,
)
...
legend = ax.legend(
    p[0:2], h[0:2], title="Meilland+ (2011)", title_fontsize=13, loc=1
)
ax.legend(
    p[2:],
    h[2:],
    title=f"Fit of the {dateText} AMBER data",
    title_fontsize=13,
    loc=4,
)
ax.add_artist(legend)
...
figwFit, axFit = fit3.simulator.plotWithResiduals(
    ["VIS2DATA", "T3PHI"], xunit="cycle/arcsec"
)
axFit[2].set_ylim(-20, 20)
