...
wl_im = np.array([2.16]) * 1e-6
pixmas = bip.getPixelSize(mas=True)

fig, ax = plt.subplots(1, len(wl_im), figsize=(5, 5))
_, _, im = m.showModel(
    bip.params["dim"].value,
    pixmas,
    fromFT=False,
    wl=wl_im,
    cmap="hot",
...
fig.tight_layout()
plt.savefig(save_dir / "ExampleBipolar_images.png")


#%%
oim.oimOptions.ft.padding=16
wl = 2.1e-6
...
v = v/v.max()
plt.figure()
plt.plot(spf, v**2 ,label="East-West")

ccf = m.getComplexCoherentFlux(spf*0, spf)
v = np.abs(ccf)
v = v/v.max()

plt.plot(spf, v**2,label="North-South")
...
vis = np.abs(m.getComplexCoherentFlux(spf_2D, spf_2D * 0, wl_2D)).reshape(
    nwl, nspf
)
...
ax.plot(
    ...
    ls=":",
    lw=5,
    label=f"Reference UD={ud.params['d'].value:.2f}mas",
)
ax.legend()
...
for i, unit in enumerate(units):
    ...
    flx_bb = (
        ...
        * np.pi
        * u.sr
        * (disc.params["Rstar"].quantity()) ** 2
        / (disc.params["dist"].quantity()) ** 2
    )
    flx_bb = flx_bb.to(
        disc.outunit, equivalencies=u.spectral_density(wl * u.m)
    )
...
vis1 = np.abs(
    morb.getComplexCoherentFlux(spf, spf0, wl=wls, t=ts.flatten())
).reshape(len(t), len(B))
vis1 /= np.outer(np.max(vis1, axis=1), np.ones(nB))
vis2 = np.abs(
    morb.getComplexCoherentFlux(spf0, spf, wl=wls, t=ts.flatten())
).reshape(len(t), len(B))
...
for i in range(nt):
    vis = np.abs(
        morb.getComplexCoherentFlux(
            spf_2D, spf_2D * 0, wl_2D, spf_2D * 0 + ts[i]
        )
    ).reshape(nwl, nspf)
    vis /= np.outer(np.max(vis, axis=1), np.ones(nspf))
    viss.append(vis)
