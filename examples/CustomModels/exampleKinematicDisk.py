...
for k in range(2):
    ...
    for i in range(3):
       #######################################################################
        ...
        ax[1+2*k][i].plot(lamsim*1e6,sim.simulatedData.data[k]['OI_VIS'].data['VISPHI'][i,:]
                          ,color="tab:blue",linewidth=lw,alpha=alpha2)
...
#    ax[0+2*k][3].plot(lamodel*1e6,spmodel-0.07,color="tab:blue",linewidth=2)
    ...
    ax[1+2*k][3].plot(lamsim*1e6,sim.simulatedData.data[k]['OI_T3'].data['T3PHI'][0,:]
                      ,color="tab:blue",linewidth=lw,alpha=alpha2)
...
m.showModel(mydim,0.025/mydim*512,wl=wl,legend=True,normalize=True,fromFT=False,normPow=0.2,colorbar=False,cmap="hot")
