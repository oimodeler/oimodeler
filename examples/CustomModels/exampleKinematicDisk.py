...
sim.data.setFilter()
fig = sim.plotWlTemplate([["VIS2DATA"],["VISPHI","T3PHI"]],xunit="micron",figsize=(12,7))
fig.set_xlim((wl0-4e-9)*1e6,(wl0+4e-9)*1e6)
fig.set_legends("$LENGTH$m $PA$$^o$","VIS2DATA",fontsize=8)
fig.set_legends(0.02,0.9,"$BASELINE$",["VIS2DATA","VISPHI","T3PHI"],fontweight =200)
...
fig,ax,im = m.showModel(mydim, 0.025/mydim*512, wl=wl, legend=True, normalize=True,
            fromFT=False, normPow=0.2, colorbar=False, cmap="plasma")
c.params["dim"].value = dim
#fig.suptitle("oimKinematicDisk",fontsize=24)
#ax[0,2].set_title("Geometrically thin kinematic disk model",fontsize=12)
