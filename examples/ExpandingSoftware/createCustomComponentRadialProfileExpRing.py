...
ring=oimExpRing(d=5,fwhm=oim.oimInterp("wl",wl=[0.5e-6,1e-6],values=[0.2,4]),elong=2,pa=30)
m=oim.oimModel(ring)
...
for iwl in range(nwl):
    cwl=iwl/(nwl-1)
    ax[0].plot(B/wl[iwl]/u.rad.to(u.mas),v[iwl,:nB//2],color=plt.cm.plasma(cwl))
    ax[1].plot(B/wl[iwl]/u.rad.to(u.mas),v[iwl,nB//2:],color=plt.cm.plasma(cwl))
