...
class oimParamLinearRangeWl(oim.oimParamInterpolator):
    def _init(
        self, param, wl0=2e-6, dwl=1e-9, values=[], kind="linear", **kwargs
    ):
...
c = oim.oimUD(
    d=oim.oimInterp(
        ...
        kind="cubic",
        dwl=5e-8,
        values=np.random.rand(nref) * 3 + 4,
    )
)
m = oim.oimModel(c)
...
# %%
wl0 = wl0 = np.linspace(
    c.params["d"].wl0.value,
    c.params["d"].wl0.value + c.params["d"].dwl.value * nref,
    num=nref,
)
...
v = np.abs(
    m.getComplexCoherentFlux(spfx_arr, spfy_arr, wl_arr).reshape(nwl, nB)
)

fig, ax = plt.subplots(2, 1)
ax[0].plot(
    wl * 1e6, c.params["d"](wl, 0), color="r", label="interpolated param"
)
...
for iB in range(1, nB):
    ax[1].plot(
        wl * 1e6, v[:, iB] / v[:, 0], color=plt.cm.plasma(iB / (nB - 1))
    )
