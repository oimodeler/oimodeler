...
class oimBox(oim.oimComponentFourier):
    ...
    def __init__(self, **kwargs):
        # NOTE: First call the __init__ function from the parent class
        super().__init__(**kwargs)
        # NOTE: Then define the component parameters
        ...
        # NOTE: Finally, call the _eval method that allows the parameters to be processed
        self._eval(**kwargs)
    ...
    def _visFunction(self, ucoord, vcoord, rho, wl, t):
        x = (
            self.params["dx"](wl, t)
            * self.params["dx"].unit.to(u.rad)
            * ucoord
        )
        y = (
            self.params["dy"](wl, t)
            * self.params["dy"].unit.to(u.rad)
            * vcoord
        )
    ...
    def _imageFunction(self, xx, yy, wl, t):
        return (
            (np.abs(xx) <= self.params["dx"](wl, t) / 2)
            & (np.abs(yy) <= self.params["dy"](wl, t) / 2)
        ).astype(float)
...
# %%
b4 = oimBox(
    dx=oim.oimInterp("wl", wl=[2e-6, 2.4e-6], values=[5, 10]),
    dy=2,
    x=20,
    y=0,
    f=0.5,
...
for i, m in enumerate(models):
    ...
    visNorth = np.abs(m.getComplexCoherentFlux(spf0, spf, wls)).reshape(
        nwl, nB
    )
