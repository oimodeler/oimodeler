...
fig2, ax2 = plt.subplots(3, 2, subplot_kw=dict(projection='oimAxes'), figsize=(8, 10))

ax2[0,0].uvplot(data.data,color="byBaseline",marker=".",legendkwargs=dict(fontsize=6))
ax2[0,1].uvplot(data.data,color="byFile",facecolor="w",legendkwargs=dict(fontsize=3.8))
ax2[1,0].uvplot(data.data,color="byConfiguration",colorTab=["r","g","b"])
ax2[1,1].uvplot(data.data,color="byArrname",colorTab=["r","g","b"],marker="+")
ax2[2,0].uvplot(data.data,label="custom label",unit="km")
ax2[2,1].uvplot(data.data,unit="cycle/rad",cunit="micron",lw=3,color="byConfiguration")
...
lamcol = ax4.oiplot(data, "SPAFREQ", "VIS2DATA", xunit="cycle/mas", label="Data",
