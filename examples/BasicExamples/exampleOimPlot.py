...
# NOTE: Using the projection='oimAsxes' for all subpltos allow to use oimPlot custom plots
fig6, ax6 = plt.subplots(
    2, 2, subplot_kw=dict(projection="oimAxes"), figsize=(8, 8)
)

# NOTE: Then we plot the V² as a function of spa. freq. with a wavelength-colorscale
ax6[0, 0].oiplot(
    data,
    "SPAFREQ",
    "VIS2DATA",
...
# NOTE: Legens also works with multicolor plots
ax6[0, 0].legend()

# NOTE: Sets log scale as for normal matplotlib plots
ax6[0, 0].set_yscale("log")


# NOTE: Plots the V² as a function of the wavlength with errorbars
# with the kwargs_error keyword allowing to pass keywords to the errorbar plot
ax6[0, 1].oiplot(
    data,
    "EFF_WAVE",
    "VIS2DATA",
...
# NOTE: Finally, we plot the closure phase with a few styling options
ax6[1, 0].oiplot(
    data,
    "SPAFREQ",
    "T3PHI",
