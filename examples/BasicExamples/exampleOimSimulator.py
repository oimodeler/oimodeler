...
# NOTE: Creating the simulator with the filename list and the model
sim = oim.oimSimulator(data=files, model=model)

# NOTE: Preparing data (building vectors of coordinates and structure of data types)
# This is automatically called when creating the simulator.
# sim.data.prepareData()

# NOTE: Computing the complex corr flux from the model at the data spatial freq
# with option to compute chi2 and final simulated data in oifits format
sim.compute(computeChi2=True, computeSimulatedData=True)

# NOTE: Printing data model chi2r
...
# NOTE: Using the dataTypes option of the oimSimulator compute method allows to
