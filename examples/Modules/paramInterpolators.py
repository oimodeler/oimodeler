...
# NOTE: Function to simplify plotting of parameter an visibility
def plotParamAndVis(B, wl, t, model, param, ax=None, colorbar=True):
    ...
    v = np.abs(
        model.getComplexCoherentFlux(
            spfx_arr, spfy_arr, wl=wl_arr, t=t_arr
        ).reshape(n, nB)
    )
...
# %%
c1 = oim.oimUD(
    d=oim.oimInterp("GaussWl", val0=2, value=4, x0=2.1656e-6, fwhm=1e-8)
)
m1 = oim.oimModel(c1)
...
# %%
c3 = oim.oimGauss(
    fwhm=oim.oimInterp("cosTime", T0=60000, P=1, values=[1, 3], x0=0.8)
)
m3 = oim.oimModel(c3)
...
plt.subplots_adjust(
    left=0.05, bottom=0.1, right=0.99, top=0.9, wspace=0.05, hspace=0.05
)
for i in range(1, 6):
...
fig.suptitle(
    "Linear, Quadratic and Cubic interpolators (with extrapolation"
