...
"""
data_dirH = path / "data" / "RealData" / "PIONIER" / "FS CMa"
data_dirL = path / "data" / "RealData" / "MATISSE" / "FSCMa"

filesH = list(data_dirH.glob("*.fits"))
filesL = list(data_dirL.glob("*.fits"))
files=filesH + filesL
"""
...
ax2[2, 0].uvplot(data.data[:11],color="b", label="Custom Label", unit="km",marker=".")

#ax2[2, 0].uvplot(data.data[:11],color="b", label="PIONIER", unit="km",marker=".")
#ax2[2, 0].uvplot(data.data[11:], color="r",label="MATISSE", unit="km",marker="o")
ax2[2, 1].uvplot(
...
#fig.set_ylim(["FLUXDATA"], 0.8, 1.3)
fig.set_xlim(2.16, 2.172)
fig.set_legends(
    0.5, 0.1, "$BASELINE$ $LENGTH$m $PA$$^o$",
    ["VIS2DATA", "VISPHI"], fontsize=9, ha="center")
