...
ax2[2, 0].uvplot(
    data.data[:11], color="b", label="Custom Label", unit="km", marker="."
)
...
# NOTE: Use the projection="oimAsxes" for all subpltos allow to use oimPlot custom plots
fig6, ax6 = plt.subplots(
    2, 2, subplot_kw=dict(projection="oimAxes"), figsize=(8, 8)
)

# NOTE: Then plot the V² as a function of spa. freq. with a wavelength-colorscale
ax6[0, 0].oiplot(
    data,
    "SPAFREQ",
    "VIS2DATA",
...
# NOTE: Legend also works with multicolor plots
ax6[0, 0].legend()

# NOTE: Set to logarithmic scale
ax6[0, 0].set_yscale("log")


# NOTE: Plot the V² as a function of the wavelength with errorbars
...
ax6[0, 1].oiplot(
    ...
    color="byBaseline",
    errorbar=True,
    kwargs_error={"alpha": 0.1},
)
ax6[0, 1].legend(fontsize=6)


# NOTE: Finally, plot the Closure phase with a few styling options
ax6[1, 0].oiplot(
    data,
    "SPAFREQ",
    "T3PHI",
...
fig.autoShape(data.data, shape=[["VIS2DATA"], ["VISPHI"], ["T3PHI"]])
...
fig.set_legends(0.5, 0.1, "$BASELINE$", ["T3PHI"], fontsize=10, ha="center")
fig.tight_layout()
