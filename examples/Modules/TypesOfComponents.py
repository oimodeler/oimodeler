...
# NOTE: Create and show images from a few components
spiral = oim.oimSpiral(dim=128, fwhm=20, P=0.01, width=0.1, pa=30, elong=2)
mspiral = oim.oimModel(spiral)

frot = oim.oimFastRotator(
    dpole=5, dim=128, incl=-50, rot=0.99, Tpole=20000, beta=0.25, pa=20
)
...
# NOTE: Check active and available FT backends
print(oim.oimOptions.ft.backend.available)
print(oim.oimOptions.ft.backend.active)

# NOTE: Change the Fourier transform (FT) backend
