...
pathData=os.path.join(os.path.join(path,os.pardir,"examples","testData","SIMPLE_TESTS"))
...
class baseTest:
    ...
    def __init__(self):
        self.setModel()
        self.data=oim.oimData(os.path.abspath(os.path.join(pathData,self.fdata)))
    ...
    def compute(self,progress=False):
        ...
        self.fit=oim.oimFitterEmcee(self.data,self.model,nwalkers=self.nwalker)
...
class pionierUD(baseTest):
    name="PIONIER UD"
    truth=np.array([10.014]) # PIONIER data generated with ASPRO have somme bias 
...
#%%        
tests=[pionierUD(),pionierBin(),gravityUDAndShiftedGauss(),matissePtAndEIRing(),matissePtAndERing()]    
ntest=len(tests) 
for itest,testi in enumerate(tests):
    if itest>=4:
        ...
        testi.makePlots(prefix=str(itest+1)+"_")
