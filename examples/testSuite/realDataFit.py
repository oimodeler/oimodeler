...
class baseTest:
    ...
    def __init__(self):
        self.setModel()
        self.data=oim.oimData(os.path.abspath(os.path.join(self.pathData,self.fdata)))
...
class matisse75Vir(baseTest):
    ...
    truth=np.array([0.45,4.5,-33.9,-79.3]) # PIONIER data generated with ASPRO have somme bias 
    chi2r0=16. # and Chi2r is not close to 1
    nwalker=50
    nstep=30000
    pathData=pathData=os.path.join(os.path.join(path,os.pardir,"examples","testData","RealData","MATISSE","binary75Vir"))
