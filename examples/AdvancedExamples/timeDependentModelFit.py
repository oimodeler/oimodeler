...
mjd_ref = [60076, 60080]
ud = oim.oimUD(
    d=oim.oimInterp("time", mjd=mjd_ref, values=[0, 0], extrapolate=True)
)
...
figWalkers, axeWalkers = fit.walkersPlot(
    cmap="plasma_r", savefig=save_dir / "complexModel_timeDependentWalkers.png"
)
figCorner, axeCorner = fit.cornerPlot(
    discard=100, savefig=save_dir / "complexModel_timeDependentCorner.png"
)
...
# %%
fig, ax = plt.subplots(
    1, 1, subplot_kw=dict(projection="oimAxes"), figsize=(8, 8)
)
timecol = ax.oiplot(
    fit.simulator.data,
    "SPAFREQ",
