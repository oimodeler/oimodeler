...
rpl = oim.oimRadialPowerLaw(dim=128, din=2, dout=5,
                            pixSize=0.1, p=0.5, pa=30, elong=2, f=0.8)

rpl.params["din"].set(min=0, max=20)
rpl.params["dout"].set(min=10, max=20)
rpl.params["p"].set(min=0, max=1)
rpl.params["elong"].set(min=1, max=50)
rpl.params["pa"].set(min=0, max=360)
rpl.params["pixSize"].free = False
rpl.params["f"].free = False

# NOTE: Model creation
model = oim.oimModel([rpl])
