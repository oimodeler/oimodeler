...
# NOTE: Model creation
star = oim.oimUD(
    d=1, f=oim.oimInterp("wl", wl=[3e-6, 4e-6], values=[0.5, 0.1])
)
ring = oim.oimESKRing(
    din=8,
    dout=oim.oimInterp("wl", wl=[3e-6, 4e-6], values=[9, 14]),
    elong=1.5,
    skw=0.8,
    pa=50,
)
...
# NOTE: Show images at the reference wavelengths (3 and 4 microns) and also at 3.5 microns
model.showModel(
    256,
    0.06,
    wl=np.linspace(3.0, 4, num=3) * 1e-6,
    legend=True,
    normalize=True,
    normPow=1,
    fromFT=True,
...
# NOTE: Export a 50 wl image cube for ASPRO
img = model.getImage(
    256, 0.06, toFits=True, fromFT=True, wl=np.linspace(3, 4, num=50) * 1e-6
)
img.writeto(product_dir / "skwDisk.fits", overwrite=True)

# NOTE: Load the simulated data from ASPRO and apply some filter to keep only
...
# NOTE: Specify the parameter space
...
params["c2_SKER_pa"].set(min=0.0, max=180)
params["c2_SKER_elong"].set(min=1, max=3)
params["c2_SKER_din"].set(min=5, max=20.0)
params["c2_SKER_skw"].set(min=0, max=1.5)
params["c2_SKER_dout_interp1"].set(min=5.0, max=30.0)
params["c2_SKER_dout_interp2"].set(min=5.0, max=30.0)

# NOTE: Perform the model-fitting
...
figSim, axSim = fit.simulator.plot(
    ["VIS2DATA", "T3PHI"], savefig=save_dir / "chromaticModelFitVisCP.png"
)
pprint(f"Chi2r = {fit.simulator.chi2r}")

# %%
figImg, axImg, im = model.showModel(
    256,
    0.06,
    wl=[3e-6, 4e-6],
    normPow=1,
