...
# NOTE: As the model has no sharp outer edge, no zero-padding is needed
oim.oimOptions.ft.padding = 1

# NOTE: Path to the AMBER oifits files: the classical Be star Alpha Col
...
# NOTE: Show 5 images in narrow bands through the disk
dwl, nwl = 32e-10, 5
wl = np.linspace(wl0 - dwl / 2, wl0 + dwl / 2, num=nwl)
fig0, _, _ = m.showModel(
    dim,
    0.05,
    wl=wl,
    legend=True,
...
# NOTE: Simulate data and compare it to the model of the disk around the
