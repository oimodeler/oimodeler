    ...
    for ifile, data in enumerate(oifitsList):
        ...
        if cname is not None:
            ...
            if cname == "MJD":
                ...
                cdata = oim.getSpaFreq(
                    data, arr=yarr, unit=cunit, squeez=False)

            elif cname == "LENGTH":
                B = oim.getBaselineLengthAndPA(
                    data, arr=yarr, squeeze=False)[0]
                cdata = []
                for idata in range(len(idx_yext)):
                ...
                PA = oim.getBaselineLengthAndPA(
                    data, arr=yarr, squeeze=False)[1]
                cdata = []
                for idata in range(len(idx_yext)):
                    cdata.append(np.transpose(
                        np.tile(PA[idata], (np.shape(data[idata].data[yname])[1], 1))))
