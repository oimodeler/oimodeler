...
class oimSimulator:
    ...
    def plotWlTemplate(self, shape, simulated=True, savefig=None, xunit="m",
            ...
            kwargsSimulatedData = {**kwargsSimulatedData0,
                                   **kwargsSimulatedData}
            ...
            fig.plot(self.simulatedData.data,
    ...
    def plotResiduals(self, arr, xaxis = 'SPAFREQ', xunit="cycle/rad",
                       savefig=None, visLog=False,cname = "EFF_WAVE", 
                       cunit="micron",cmap="plasma",marker=".",levels=[1,2,3], **kwargs):
            ...
            if type(levels)!=type(None):
                ...
                for j,leveli in enumerate(levels):
                    for k in range(2):
                        ...
                        ax[i].plot(xlim,y,ls="--",color="grey",alpha=alpha[j])            
    ...
    def plotWithResiduals(self, arr, simulated=True, savefig=None, visLog=False, 
                          xaxis="SPAFREQ",xunit="cycle/rad", cname = "EFF_WAVE",
                          cunit="micron", cmap="plasma", kwargsData={},
                          kwargsSimulatedData={}, kwargsResiduals={},levels=[1,2,3]):
        ...
        if not("ls" in kwargsResiduals) and not("linestyle" in kwargsResiduals):
            kwargsResiduals["ls"]=""
        ...
        fig, ax = plt.subplots(len(arr)*2, 1, sharex=True, figsize=(8, 6),
                               subplot_kw=dict(projection='oimAxes'),
        ...
        for i in range(nplots):
            # NOTE: Plotting the data with wavelength colorscale + errorbars vs
            # spatial frequencies
            scale = ax[2*i].oiplot(self.data.data, xaxis, arr[i],
                               xunit=xunit, showColorbar=False, **kwargsData)

            # NOTE: Over-plotting the simulated data as a dotted line vs spatial
            # frequencies
            ax[2*i].oiplot(self.simulatedData.data, xaxis, arr[i],
                       xunit=xunit, **kwargsSimulatedData)
        ...
        for i in range(nplots):
            ...
            scale =ax[2*i+1].oiplot(residuals_data, xaxis, arr[i], xunit=xunit,
                                showColorbar=False, **kwargsResiduals)
            ...
            if type(levels)!=type(None):
                ...
                for j,leveli in enumerate(levels):
                    for k in range(2):
                        y=np.array([1,1])*(2*k-1)*(leveli)
                        ax[2*i+1].plot(xlim,y,ls="--",color="grey",alpha=alpha[j])            
            #ax[i].set_title(label_p)
