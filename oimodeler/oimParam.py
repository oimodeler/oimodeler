...
class oimParamInterpolatorKeyframes(oimParamInterpolator):
    ...
    def _interpFunction(self,wl,t):
        ...
        if self.dependence=="wl":
            ...
            var=t
        values=np.array([pi.value for pi in self.keyvalues])
        keyframes=np.array([pi.value for pi in self.keyframes])
...
class oimParamCosineTime(oimParamInterpolator):
    ...
    def _init(self,param,T0=0,P=1,values=[0,1],x0=None,**kwargs):
        ...
        self.T0=oimParam(name="T0",value=T0,description="Start",unit=units.day)
        self.P=oimParam(name="P",value=P,description="Period",unit=units.day)
        ...
        if x0!=None:
            self.x0=oimParam(name="x0",value=x0,description="Inflection point",unit=units.one)
    ...
    def _interpFunction(self,wl,t):
        ...
        if self.assymetric==True:
            normt=interp1d([0,self.x0.value,1],[0,0.5,1],kind='slinear')(normt)
        
        return (np.cos(normt*2*np.pi)+1)/2*(self.values[0].value-
...
class oimParamGaussian(oimParamInterpolator):
    ...
    def _interpFunction(self,wl,t):
        ...
        return self.offset.value+self.height.value*\
            np.exp(-2.77*(var-self.x0.value)**2/self.fwhm.value**2)
