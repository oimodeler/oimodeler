...
bandnames = np.array(["B","V","R","I","J","H","K","L","M","N"])
wl_bands  = np.array([445,551,658,806,1220,1630,2190,3450,4750,10500])*1e-9
...
def getJSDCDiameter(name,bandOrWl):
    ...
    if type(bandOrWl)==str:
        ...
        band = bandnames[np.argmin(np.abs(wl_bands-bandOrWl))]    
...
def oimCalibrate(sci,cal,D=None,eD=None):
    ...
    if not(D):
        ...
        print(f"No diameter given for cal {calname} at wl={wlmean*1e6:.1f}um," \
              " searching in JSDC...")
    ...
    for iarr in range(len(data_sci.data[0])):
        ...
        if  arrname=="OI_VIS2":
            ...
            #-------------------------VIS2DATA---------------------------------
            data_cal.data[0]["OI_VIS2"].data["VIS2DATA"] /= \
                        sim.simulatedData.data[0]["OI_VIS2"].data["VIS2DATA"]
            data_sci.data[0]["OI_VIS2"].data["VIS2DATA"] /= \
                        data_cal.data[0]["OI_VIS2"].data["VIS2DATA"]           
        ...
        if  arrname=="OI_VIS":
            ...
            #---------------------------VISAMP---------------------------------
            if True: #TODO put here a check on the presence of VISAMP data and the kind of DATA 
                data_cal.data[0]["OI_VIS"].data["VISAMP"] /= \
                        sim.simulatedData.data[0]["OI_VIS"].data["VISAMP"]
                data_sci.data[0]["OI_VIS"].data["VISAMP"] /= \
                        data_cal.data[0]["OI_VIS"].data["VISAMP"]
            ...
            #--------------------------VISPHI---------------------------------
            phaser_model = _phaser(sim.simulatedData.data[0]["OI_VIS"].data["VISPHI"])
        ...
        if  arrname=="OI_T3":
            ...
            #----------------------------T3AMP---------------------------------
            if False: #TODO put here a check on the presence of VISAMP data and the kind of DATA 
                data_cal.data[0]["OI_T3"].data["T3AMP"] /= \
                            sim.simulatedData.data[0]["OI_T3"].data["T3AMP"]
                data_sci.data[0]["OI_T3"].data["T3AMP"] /= \
                            data_cal.data[0]["OI_T3"].data["T3AMP"] 
            
            #----------------------------T3PHI---------------------------------
            phaser_model = _phaser(sim.simulatedData.data[0]["OI_T3"].data["T3PHI"])
        ...
        if  arrname=="OI_FLUX":
                ...
                #--------------------------FLUXDATA----------------------------
                data_cal.data[0]["OI_FLUX"].data["FLUXDATA"] /= \
                        sim.simulatedData.data[0]["OI_FLUX"].data["FLUXDATA"]
                data_sci.data[0]["OI_FLUX"].data["FLUXDATA"] /= \
                        data_cal.data[0]["OI_FLUX"].data["FLUXDATA"]
