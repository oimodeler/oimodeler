...
angToSize = lambda x: (
    units.AU,
    units.mas,
    lambda y: 1000 * y / x.to(units.parsec).value,
    lambda y: y / 1000 * x.to(units.parsec).value,
)
...
class oimKinematicDisk(oimComponentImage):
    ...
    def __init__(self, **kwargs):
        ...
        self.params["fov"] = oimParam(
            name="fov",
            value=30,
            description="Field of view in stellar diameters",
            unit=units.one,
            free=False,
        )
        self.params["wl0"] = oimParam(
            name="wl0",
            value=2.1656e-6,
            description="central wavelength of the line",
            unit=units.m,
            free=False,
        )
        self.params["dwl"] = oimParam(
            name="dwl",
            value=0.9e-10,
            description="pixel size in wavelength ",
            unit=units.m,
            free=False,
        )
        self.params["res"] = oimParam(
            name="R",
            value=1.8e-10,
        ...
        self.params["dist"] = oimParam(
            name="dist",
            value=100,
            description="distance in parsec",
            unit=units.pc,
            free=False,
        )
        self.params["fwhmCont"] = oimParam(
            name="fwhmCont",
            value=5,
            description="FWHM of the disk in the continuum in DStar",
            unit=units.one,
        )
        self.params["fwhmLine"] = oimParam(
            name="fwhmLine",
            value=2,
            description="FWHM of the disk in the line in DStar",
            unit=units.one,
        )
        self.params["fluxDiskCont"] = oimParam(
            name="fluxDiskCont",
            value=0.5,
            description="Flux of the disk in the continuum",
            unit=units.one,
        )
        ...
        self.params["EW"] = oimParam(
            name="EW",
            value=40,
            description="Equivalent width of the line",
            unit=units.AA,
        )
        self.params["vrot"] = oimParam(
            name="vrot",
            value=400,
            description="rotational velocity at the photosphere",
            unit=units.km / units.s,
        )
        self.params["beta"] = oimParam(
            name="beta",
            value=-0.5,
            description="exponent of the rotational law",
            unit=units.one,
        )
        self.params["v0"] = oimParam(
            name="v0",
            value=0,
            description="expension velocity at the photosphere",
            unit=units.km / units.s,
            free=False,
        )
        self.params["vinf"] = oimParam(
            name="vinf",
            value=0,
            description="expension velocity at the infinity ",
            unit=units.km / units.s,
            free=False,
        )
        self.params["gamma"] = oimParam(
            name="vinf",
            value=0.86,
            description="exponent of the expansion velocity law",
            unit=units.one,
            free=False,
        )
    ...
    def _internalImage(self):
        ...
        incl = self.params["incl"].value * self.params["incl"].unit.to(
            units.rad
        )
        ...
        self._wl = np.linspace(
            wl0 - dwl * (nwl // 2), wl0 + dwl * (nwl // 2), num=nwl
        )
        ...
        vr = np.abs(v0 + (vinf - v0) * (1.0 - 1 / r_incl)) ** gamma
        ...
        mapEnvL = np.exp((-(r_incl**2)) / (2 * sigmaLine**2.0)) * (r > 1)
        mapEnvL = mapEnvL / np.sum(mapEnvL)
        mapEnvC = np.exp((-(r_incl**2)) / (2 * sigmaCont**2.0))
        ...
        for iwl in range(nwl):
            mapL[0, iwl, :, :] = (
                mapC + np.exp(-((vmap - v[iwl]) ** 2.0) / C0) * C
            )
        return mapL
