...
from ..oimParam import _standardParameters, oimParam, oimParamNorm
...
def keplerCoordinates(phase, eccentricity, majorAxis):
    ...
    theta = 2 * np.arctan(
        np.tan(phase / 2.0) * np.sqrt((1 + eccentricity) / (1 - eccentricity))
    )
    X = rho * np.cos(theta)
    Y = rho * np.sin(theta)
    XY = np.array([X, Y])
...
def binaryCoordinates(time, eccentricity, majorAxis, period):
    ...
    for k in range(1, order):
        phi = phi - 2 * special.jn(float(k), k * eccentricity) / float(
            k
        ) * np.sin(float(k) * omega * time)
    XY = keplerCoordinates(phi, eccentricity, majorAxis)
    return XY


def binaryProjectedCoordinates(
    time, t0, eccentricity, majorAxis, period, i, omega, OMEGA
):
...
def radialVelocity(time, T, t0, e, omega, Ka, V0):
    ...
    nu = 2 * np.arctan(np.sqrt((1 + e) / (1 - e)) * np.tan(EE / 2.0))
...
class oimBinaryOrbit(oimComponentFourier):
    ...
    def __init__(self, **kwargs):
        ...
        self._params["T"] = oimParam(
            name="T", value=0, description="Period", unit=u.day, free=True
        )
        self._params["T0"] = oimParam(
            name="T0",
            value=0,
            description="Epoch of periastron passage (MJD)",
            unit=u.day,
            free=True,
            mini=0,
        )
        self._params["i"] = oimParam(
            name="i",
            value=0,
    ...
    def normalizeFlux(self, value):
        ...
        if value == False and isinstance(
            self.secondary.params["f"], oimParamNorm
        ):
            self.secondary.params["f"] = self._params["f"] = oimParam(
                **_standardParameters["f"]
            )
        elif value == True and not (
            isinstance(self.secondary.params["f"], oimParamNorm)
        ):
            self.secondary.params["f"] = oimParamNorm(self.primary.params["f"])
    ...
    def _visFunction(self, ucoord, vcoord, rho, wl, t):
        ...
        s = (
            self.secondary.getComplexCoherentFlux(ucoord, vcoord, wl, t)
            * transFact
        )
        return p + s
