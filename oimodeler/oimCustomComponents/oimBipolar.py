...
def bipolar(wl0, res, dim, incl, vpole, veq, alpha, beta, F0, time, dr, dist):
    ...
    mapp = (np.abs(vri * t_seconds - r) < dr * pix_km) * 1.0 / r**beta
    ...
    for iv in range(dimv):
        ...
        mapi = (
            np.exp(-((vproj - v[iv]) ** 2.0) / (2 * (resv / 2.3548) ** 2.0))
            * mapp
            / C2
        )
        # Vmapi=mapi*(zp>0)
...
class oimBipolar(oimComponentImage):
    ...
    def __init__(self, **kwargs):
        ...
        self.params["veq"] = oimParam(
            name="veq",
            value=500,
            description="Expansion velocity at the equator",
            unit=units.km / units.s,
        )
        self.params["vpole"] = oimParam(
            name="vpole",
            value=2000,
            description="Expansion velocity at the vpole",
            unit=units.km / units.s,
        )
        self.params["alpha"] = oimParam(
            name="alpha",
            value=1,
            description="exponent of the expansion velocity ",
            unit=units.one,
        )
        self.params["beta"] = oimParam(
            name="beta",
            value=-3,
            description="exponent of radial intensity profile ",
            unit=units.one,
        )

        self.params["time"] = oimParam(
            name="time",
            value=1,
            description="Time (in days) aftert the explosion",
            unit=units.day,
        )

        self.params["dr"] = oimParam(
            name="dr",
            value=5,
            description="width of the nebula (in pixel)",
            unit=units.one,
            free=False,
        )
    ...
    def _internalImage(self):
        ...
        im, wl, pix_rad = bipolar(
            wl0, res, dim, incl, vpole, veq, alpha, beta, EW, time, dr, dist
        )
        im, wl, pix_rad = bipolar(
            wl0, res, dim, incl, vpole, veq, alpha, beta, EW, time, dr, dist
        )
